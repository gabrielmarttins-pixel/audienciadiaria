...
def render_html(data: dict) -> str:
    ...
    data["assets"] = {"plim": image_data(PLIMPLIM), "plimWhite": image_data(PLIMPLIM_WHITE), "inteligencia": image_data(INTELIGENCIA), "conf": image_data(CONFIDENTIAL)}
...
<header class="app"><div class="top"><div class="brand"><img class="brand-logo" id="plimLogo" src="{plim_icon}" alt="Plim plim"><span>Desempenho Diário</span></div><div class="header-right"><div class="actions"><input id="baseUpload" type="file" accept=".zip,.xlsx" multiple hidden><button class="action-btn primary" id="uploadBtn" type="button">Enviar base</button><span class="file-status" id="fileStatus"></span></div><nav class="tabs"><button class="tab active" type="button" data-tab="resumo" onclick="activateTab(this)">RESUMO</button><button class="tab" type="button" data-tab="programas" onclick="activateTab(this)">PROGRAMAS</button><button class="tab" type="button" data-tab="perfil" onclick="activateTab(this)">PERFIL</button></nav></div></div></header>
<main>
...
async function downloadVisualPdf(name){{
  ...
  function leader(ctx,x,y,w,h,item,maxLead){{const c=channelMap[item.key]||item;card(ctx,x,y,w,h,c.color,'#fff');mark(ctx,c,x+20,y+16,42);text(ctx,c.label,x+72,y+32,17,'#607086','700');text(ctx,item.minutes+' min',x+72,y+64,25,'#101521','700');text(ctx,fmt(item.percent,1)+'% | '+item.hours,x+w-22,y+64,17,c.color,'700','right');ctx.fillStyle='#e8eef7';ctx.beginPath();ctx.roundRect(x+20,y+h-20,w-40,8,4);ctx.fill();ctx.fillStyle=c.color;ctx.beginPath();ctx.roundRect(x+20,y+h-20,(w-40)*Math.max(0,Math.min(1,item.minutes/(maxLead||1))),8,4);ctx.fill()}}
  function rankingMini(ctx,x,y,w,h,c,rows){{card(ctx,x,y,w,h,c.color,'#fff');text(ctx,c.label,x+34,y+42,22,c.color,'700');rows.slice(0,5).forEach((r,i)=>{{const yy=y+78+i*31;const dot=r.shareVar==null?'#9aa4b2':r.shareVar>=0?'#20c96b':'#ef3340';ctx.fillStyle=dot;ctx.beginPath();ctx.arc(x+38,yy-6,6,0,Math.PI*2);ctx.fill();const f=fit(ctx,(i+1)+'. '+r.program,w-205,15,'700');text(ctx,f.txt,x+54,yy,f.size,'#101521','700');const varText=r.shareVar==null?'-':fmt(r.shareVar,1)+'%';text(ctx,fmt(r.share,2)+'% | '+varText,x+w-42,yy,14,'#607086','700','right')}})}}
  function rankingWide(ctx,x,y,w,h,c,rows){{card(ctx,x,y,w,h,c.color,'#fff');const left=x+52,right=x+w-58,audX=x+w-350,shareX=x+w-220; text(ctx,c.label,left,y+44,25,c.color,'700');text(ctx,'Programa',left,y+84,12,'#607086','700');text(ctx,'Aud.',audX,y+84,12,'#607086','700','right');text(ctx,'Share',shareX,y+84,12,'#607086','700','right');text(ctx,'Var. SHR',right,y+84,12,'#607086','700','right');rows.slice(0,8).forEach((r,i)=>{{const yy=y+116+i*24;const dot=r.shareVar==null?'#9aa4b2':r.shareVar>=0?'#20c96b':'#ef3340';ctx.fillStyle=dot;ctx.beginPath();ctx.arc(left,yy-7,6.5,0,Math.PI*2);ctx.fill();const f=fit(ctx,(i+1)+'. '+r.program,w-500,15,'700');text(ctx,f.txt,left+22,yy,f.size,'#101521','700');text(ctx,fmt(r.aud,2),audX,yy,14,'#101521','700','right');text(ctx,fmt(r.share,2)+'%',shareX,yy,14,c.color,'700','right');text(ctx,r.shareVar==null?'-':fmt(r.shareVar,1)+'%',right,yy,14,dot,'700','right')}})}}
  ...
  section(ctx,'Minutos na liderança',1090);
  const maxLead=Math.max(1,...(DATA.leadership||[]).map(l=>l.minutes||0));
  (DATA.leadership||[]).forEach((l,i)=>leader(ctx,54+(i%2)*592,1140+Math.floor(i/2)*118,540,94,l,maxLead));
...
async function exportHtml(){{
  ...
  clone.querySelectorAll('.final-actions,.highlights-editor').forEach(el=>el.remove());
  ...
  const compatScript=clone.ownerDocument.createElement('script');
  compatScript.textContent="(function(){{function hasClass(el,c){{return (' '+(el.className||'')+' ').indexOf(' '+c+' ')>-1}}function addClass(el,c){{if(!hasClass(el,c))el.className=(el.className?el.className+' ':'')+c}}function removeClass(el,c){{el.className=(' '+(el.className||' ')+' ').replace(' '+c+' ',' ').replace(/^\\s+|\\s+$/g,'')}}function closest(el,selectorClass){{while(el&&el.nodeType===1){{if(hasClass(el,selectorClass))return el;el=el.parentNode}}return null}}document.addEventListener('click',function(e){{var tab=closest(e.target,'tab');if(!tab||!tab.getAttribute('data-tab'))return;var id=tab.getAttribute('data-tab');var tabs=document.querySelectorAll('.tabs .tab');for(var i=0;i<tabs.length;i++)removeClass(tabs[i],'active');var panels=document.querySelectorAll('.panel');for(var j=0;j<panels.length;j++)removeClass(panels[j],'active');addClass(tab,'active');var panel=document.getElementById(id);if(panel)addClass(panel,'active')}},false)}})();";
  clone.querySelector('body').appendChild(compatScript);
