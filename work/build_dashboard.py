...
function renderDailyInsights(selectedKey='globo'){{
  ...
  target.querySelectorAll('.insight-tab').forEach(btn=>btn.addEventListener('click',()=>renderDailyInsights(btn.dataset.insight)));
}}
...
function buildDailyInsights(selectedKey='globo'){{
  ...
  const tabs=channels.map(c=>{{
    ...
    return `<button class="insight-tab ${{active?'active':''}}" data-insight="${{c.key}}" style="${{style}}">${{logoHtml(c)}}${{c.label}}</button>`;
  }}).join('');
  ...
  if(selected.key==='nic'){{
    ...
    headline=selected.key==='globo'
      ? `A Globo fechou o per\u00edodo 07h-24h com m\u00e9dia de ${{fmt(current.aud,2)}} pontos e ${{fmt(current.share,2)}}% de share. A an\u00e1lise indica ${{fall&&fall.audVar<0?'queda em parte da grade':'estabilidade na grade'}} na compara\u00e7\u00e3o com as \u00faltimas 4 semanas, com ${{leader.minutes}} minutos de lideran\u00e7a (${{fmt(leader.percent,1)}}% do per\u00edodo).`
...
function exportHtml(){{const clone=document.documentElement.cloneNode(true);clone.querySelectorAll('#baseUpload').forEach(el=>el.value='');clone.querySelectorAll('.final-actions').forEach(el=>el.remove());clone.querySelectorAll('.tab').forEach(btn=>btn.classList.toggle('active',btn.dataset.tab==='resumo'));clone.querySelectorAll('.panel').forEach(panel=>panel.classList.toggle('active',panel.id==='resumo'));const cloneBody=clone.querySelector('body');if(cloneBody)cloneBody.className=cloneBody.className.replace(/\\bawaiting-bases\\b/g,'').trim();const dataScript=clone.querySelector('#data');if(dataScript)dataScript.textContent=JSON.stringify(DATA).replaceAll('</','<\\/');downloadBlob(`desempenho_diario_df_${{dateToken()}}.html`,'text/html;charset=utf-8','<!doctype html>\\n'+clone.outerHTML)}}
