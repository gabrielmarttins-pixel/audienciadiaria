...
function buildDataFromBooks(books){{
  ...
  const currentRank=byName('Base Ranking diario_REC')||byName('Base Ranking di?rio_REC');
  const previousRank=byName('Base Ranking diario_DF7d')||byName('Base Ranking di?rio_DF7d');
  ...
  const dayMinutes=minutesAll.filter(m=>m.minute>=7*60&&m.minute<24*60);
  ...
  const line=dayMinutes.map(m=>({{time:m.time,aud:Object.fromEntries(DATA.channels.map(ch=>[ch.key,pctJs(m.aud[ch.key])]))}}));
  ...
  for(const p of programs){{
    ...
    for(const ch of DATA.leadershipChannels){{
      const targetKey=Object.keys(competition[p.name]||{{}}).find(t=>normalizeText(t).includes('total domic'))||'Total Domic?lios';const metrics=competition[p.name]?.[targetKey]?.[ch.key]||{{}};
  ...
  const weekdays=['domingo','segunda-feira','ter?a-feira','quarta-feira','quinta-feira','sexta-feira','s?bado'];
