...
async function downloadVisualPdf(name){{
  ...
  function rankingMini(ctx,x,y,w,h,c,rows){{card(ctx,x,y,w,h,c.color,'#fff');text(ctx,c.label,x+34,y+42,22,c.color,'700');rows.slice(0,5).forEach((r,i)=>{{const yy=y+78+i*31;const st=r.shareStatus||statusFromVar(r.shareVar);const dot=st==='estavel'?'#9aa4b2':st==='cresceu'?'#20c96b':'#ef3340';ctx.fillStyle=dot;ctx.beginPath();ctx.arc(x+38,yy-6,6,0,Math.PI*2);ctx.fill();const f=fit(ctx,(i+1)+'. '+r.program,w-205,15,'700');text(ctx,f.txt,x+54,yy,f.size,'#101521','700');const varText=r.shareVar==null?'-':fmt(r.shareVar,1)+'%';text(ctx,fmt(r.share,2)+'% | '+varText,x+w-42,yy,14,'#607086','700','right')}})}}
  function rankingWide(ctx,x,y,w,h,c,rows){{card(ctx,x,y,w,h,c.color,'#fff');const left=x+52,right=x+w-58,audX=x+w-350,shareX=x+w-220; text(ctx,c.label,left,y+44,25,c.color,'700');text(ctx,'Programa',left,y+84,12,'#607086','700');text(ctx,'Aud.',audX,y+84,12,'#607086','700','right');text(ctx,'Share',shareX,y+84,12,'#607086','700','right');text(ctx,'Var. SHR',right,y+84,12,'#607086','700','right');rows.slice(0,8).forEach((r,i)=>{{const yy=y+116+i*24;const st=r.shareStatus||statusFromVar(r.shareVar);const dot=st==='estavel'?'#9aa4b2':st==='cresceu'?'#20c96b':'#ef3340';ctx.fillStyle=dot;ctx.beginPath();ctx.arc(left,yy-7,6.5,0,Math.PI*2);ctx.fill();const f=fit(ctx,(i+1)+'. '+r.program,w-500,15,'700');text(ctx,f.txt,left+22,yy,f.size,'#101521','700');text(ctx,fmt(r.aud,2),audX,yy,14,'#101521','700','right');text(ctx,fmt(r.share,2)+'%',shareX,yy,14,c.color,'700','right');text(ctx,r.shareVar==null?'-':fmt(r.shareVar,1)+'%',right,yy,14,dot,'700','right')}})}}
